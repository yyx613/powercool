...
class SaleController extends Controller
{
    ...
    public function convertToInvoice(Request $req)
    {
        ...
        try {
            ...
            $jsonFilename = $sku . '.json';

            // 调用生成 JSON 的函数获取 JSON 内容
            $jsonContent = $this->generateJsonInvoice($inv, $do_products, $dos);

            // 使用 Storage 存储 JSON 文件
            Storage::put(self::INVOICE_PATH . $jsonFilename, $jsonContent);
    ...
    private function generateJsonInvoice($invoice, $doProducts, $deliveryOrders)
    {
        $invoiceData = [
        ...
        return json_encode($invoiceData, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    }
