...
class SaleController extends Controller
{
    ...
    {
        // Start building the query with joins to access related fields
        $records = Invoice::select('invoices.id', 'invoices.sku', 'invoices.filename', 'delivery_orders.customer_id')
            ->leftJoin('delivery_orders', 'delivery_orders.invoice_id', '=', 'invoices.id'); // Join deliveryorders to access customer_id
        ...
        foreach ($records_paginator as $record) {
            $data['data'][] = [
                ...
                'sku' => $record->sku,
                'filename' => $record->filename,
                'customerId' => $record->customer_id, // Directly using customer_id from the join
            ];
        }
