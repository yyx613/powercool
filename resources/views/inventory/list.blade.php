...
@section('content')
    <div class="mb-3 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-2 md:mb-0" description="{{ __('Track stock levels, categories, and product details') }}">{{ __($title) }}</x-app.page-title>
        ...
        <div class="flex flex-wrap items-center gap-x-4 max-w-screen-sm w-full mb-4">
            <div class="flex-1">
                <x-app.input.input name="filter_search" id="filter_search" class="flex items-center"
                    placeholder="{{ __('Search') }}" value="{{ $default_search ?? '' }}">
    ...
    <x-app.modal.raw-material-transfer-modal hasFactory="{{ $is_production ? false : true }}" />
...
@push('scripts')
    <script>
                ...
                render: function(data, type, row) {
                    switch (data) {
                        case 0:
                            return "{!! __('Inactive') !!}"
                        case 1:
                            if (IS_PRODUCTION == true && row.is_sparepart == false) {
                                return `{!! __('In Transit') !!} (${row.stock_out_factory})`
                            }
                            return "{!! __('Active') !!}"
                        case 2:
                ...
                render: function(data, type, row) {
                    ...
                    return IS_PRODUCTION ?
                        `<div class="flex items-center justify-end gap-x-2 px-2">
                            ${
                                row.is_sparepart == true || row.has_factory_child ?
                            ...
                            ${
                                row.is_sparepart == false ? `
                                        ${
                                            row.status === 1 ? `
                                        <button type="button" class="rounded-full p-2 bg-red-200 flex items-center reject-btns" data-frm-id="${row.frm_id}" title="{!! __('Reject') !!}">
                                        ...
                                        <button type="button" class="rounded-full p-2 bg-green-200 flex items-center accept-btns" data-frm-id="${row.frm_id}" title="{!! __('Accept') !!}">
        ...
        var dt = new DataTable('#data-table', {
            ...
            "search": {
                "search": "{{ isset($default_search) && $default_search != null ? $default_search : null }}"
            },
            ajax: {
                data: function() {
        ...
        $('#raw-material-transfer-modal #yes-btn').on('click', function() {
            ...
            url =
                `${url}?product_id=${PRODUCT_ID_TO_TRANSFER}&qty=${qty}&frm_id=${FRM_ID_TO_TRANSFER}&factory=${$('#raw-material-transfer-modal select[name="factory"]').val()}`
        ...
        $('body').on('click', '.reject-btns', function() {
            ...
            let url = "{{ config('app.url') }}"
            url = `${url}/inventory-category/reject-production-stock-out-rm/${frmId}`
            $('#approval-reject-modal #yes-btn').attr('href', url)
        })
        $('body').on('click', '.accept-btns', function() {
            let frmId = $(this).data('frm-id')

            $('#confirmation-modal #msg').text(`{!! __('Are you sure you want to accept') !!} ?`)
            $('#confirmation-modal').addClass('show-modal')

            let url = "{{ config('app.url') }}"
            url = `${url}/inventory-category/accept-production-stock-out-rm/${frmId}`
