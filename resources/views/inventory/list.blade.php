...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        <x-app.page-title>{{ $is_product ? 'Product' : 'Raw Material' }}</x-app.page-title>
        <a href="{{ $is_product ? route('product.create') : route('raw_material.create') }}" class="bg-yellow-400 shadow rounded-md py-2 px-4 flex items-center gap-x-2">
            ...
            { 
                ...
                render: function (data, type, row) {
                    return  `<div class="flex items-center justify-end gap-x-2 px-2">
                        ${
                            IS_PRODUCT == true || (IS_PRODUCT == false && row.is_sparepart == true) ? `
