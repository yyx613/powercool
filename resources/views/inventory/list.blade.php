...
@section('content')
    <div class="mb-6 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-4 md:mb-0">{{ __($is_product ? 'Product' : ($is_production ? 'Production Material' : 'Raw Material')) }}</x-app.page-title>
...
@push('scripts')
    <script>
                ...
                render: function (data, type, row) {
                    return IS_PRODUCTION ?
                        `<div class="flex items-center justify-end gap-x-2 px-2">
                             <a href="{{ config('app.url') }}/production-material/view/${row.id}" class="rounded-full p-2 bg-green-200 inline-block" title="{!! __('View') !!}">
        ...
        $('#export-btn').on('click', function() {
            window.location.href = IS_PRODUCT ? '{{ route("product.export") }}' : '{{ route("raw_material.export") }}'
        })
    </script>
@endpush
