        ...
        @if (isset($prod))
            <div class="bg-white p-4 border rounded-md mt-6" id="milestone-container">
                <div class="flex justify-between mb-2">
                    <x-app.input.label class="mb-2">{{ __('Milestones') }}</x-app.input.label>
            ...
            <ul class="my-2" id="serial_no_list">
                ...
                <li class="hidden group flex items-center rounded hover:bg-slate-100" id="serial-no-template">
                    ...
                    <div class="flex justify-between w-full">
                        ...
                        <button type="button"
                            class="bg-rose-400 p-1.5 rounded text-white text-xs font-semibold opacity-0 group-hover:opacity-100 delete-serial-no-btns"
                            title="Remove">
                            Remove
                        </button>
                    </div>
    ...
    <script>
        ...
        $('#form').one('submit', function(e) {
            ...
            for (const key in MILESTONES) {
                if (MILESTONES[key].is_checked) {
                    temp[key] = MILESTONES[key].material_use_product_ids
                }
            }

            $('input[name="milestones"]').val(JSON.stringify(temp))
        ...
        $('body').on('change', '.first-half input', function() {
            ...
            if (MILESTONES[milestoneId] == undefined) {
        ...
        function getMilestones() {
            ...
            url = `${url}/milestone/get/${categoryId}/${typeId}`
            ...
            $.ajax({
                ...
                success: function(res) {
                    for (let i = 0; i < res.milestones.length; i++) {
                        ...
                        $(clone).find('.first-half input').attr('id', `ms-${ms.id}`)
                    ...
                    for (let i = 0; i < PRODUCT.milestones.length; i++) {
                        ...
                        MILESTONES[PRODUCT.milestones[i].milestone_id] = {
                            material_use_product_ids: PRODUCT.milestones[i]
