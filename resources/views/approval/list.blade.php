...
@section('content')
    ...
    <div>
        ...
        <div class="flex justify-between items-center mb-4">
            <div class="flex gap-x-2 w-full max-w-3xl">
                <div class="flex-1">
...
@push('scripts')
    <script>
        ...
        DEFAULT_TYPE = @json($default_type ?? null);
        TABLE_FILTER = {
        ...
        var dt = new DataTable('#data-table', {
                    ...
                    render: function(data, type, row) {
                        if (data != null) {
                            if (data.includes('Customer')) {
                                return 'Customer';
                            } else if (data.includes('MaterialUse')) {
                                return 'B.O.M Request';
                            } else if (data.includes('Production')) {
                                return 'Complete Production Request';
                            } else if (data.includes('FactoryRawMaterial')) {
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        if (row.debtor_name != null && row.debtor_code != null) {
                            display =
                                `${display}<br><span class="text-xs">Debtor: ${row.debtor_name} (${row.debtor_code})</span>`
                        }
                        if (row.sales_agent_name != null) {
                            display =
                                `${display}<br><span class="text-xs">Sales Agent: ${row.sales_agent_name}</span>`
                        }
                        return display
                    }
                },
            ...
            ajax: {
                data: function() {
                    ...
                    url =
                        `${url}?page=${ INIT_LOAD == true && DEFAULT_PAGE != null ? DEFAULT_PAGE : info.page + 1 }&status=${ TABLE_FILTER['status'] }&type=${ TABLE_FILTER['type'] }`
        ...
        $('body').on('click', '.reject-btns', function() {
            ...
            $('#approval-reject-modal').attr('data-id', id)
            $('#approval-reject-modal').addClass('show-modal') 
        })
        // Submit reject reason
        $('body').on('click', '#approval-reject-modal #yes-btn', function() {
            let id = $('#approval-reject-modal').attr('data-id')
            let url = '{{ config('app.url') }}'
            ...
            $.ajax({
                ...
                url: url,
                data: {
                    remark: $('#approval-reject-modal textarea').val(),
                },
                type: 'POST',
                success: function(res) {
