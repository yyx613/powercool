...
@section('content')
    <div class="mb-3 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-2 md:mb-0" description="{{ __('Define user roles and configure access permissions') }}">{{ __('Role Management') }}</x-app.page-title>
        @can('user_role_management.create')
        <a href="{{ route('role_management.create') }}" class="bg-yellow-400 shadow rounded-md py-2 px-4 flex items-center gap-x-2">
                            ...
                            for (let i = 0; i < res.length; i++) {
                                res[i].action = `
                                    <div class="flex items-center justify-end gap-x-2 px-2">
                                        ...
                                        <a href="{{ config('app.url') }}/role-management/edit/${res[i].id}" class="rounded-full p-2 bg-blue-200 inline-block" title="{!! __('Edit') !!}">
