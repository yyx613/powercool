...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        ...
        <a href="{{ route('role_management.create') }}" class="bg-yellow-400 text-white shadow rounded-md py-2 px-4 flex items-center gap-x-2">
...
@push('scripts')
    <script>
                            ...
                            for (let i = 0; i < res.length; i++) {
                                res[i].action = `
                                    <div class="flex items-center justify-end gap-x-2 px-2">
                                        ...
                                        <button class="rounded-full p-2 bg-red-200 inline-block delete-btns" data-id="${res[i].id}" data-name="${res[i].role}">
        ...
        $('#role-table').on('click', '.delete-btns', function() {
            id = $(this).data('id')
            name = $(this).data('name')

            $('#delete-modal #yes-btn').attr('href', `{{ config('app.url') }}/role-management/delete/${id}`)
            $('#delete-modal #role-name').text(name)
