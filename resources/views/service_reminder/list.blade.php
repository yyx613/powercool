@extends('layouts.app')
@section('title', 'Service Reminder')
...
@section('content')
    <div class="mb-3 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-2 md:mb-0" description="{{ __('Track upcoming and overdue service schedules by product') }}">{{ __('Service Reminder') }}</x-app.page-title>
        @can('service_reminder.create')
        <a href="{{ route('service_reminder.create') }}" class="bg-yellow-400 shadow rounded-md py-2 px-4 flex items-center gap-x-2">
        ...
        var dt = new DataTable('#data-table', {
                ...
                { 
                    ...
                    render: function (data, type, row) {
                       return  `<div class="flex items-center justify-end gap-x-2 px-2">
                            <a href="{{ config('app.url') }}/service-reminder/view/${row.id}" class="rounded-full p-2 bg-green-200 inline-block" title="{!! __('View Reminder') !!}">
            ...
            ajax: {
                data: function(){
                    ...
                    var url = "{{ route('service_reminder.get_data') }}"
