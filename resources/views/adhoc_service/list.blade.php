...
@section('content')
    <div class="mb-3 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-2 md:mb-0" description="{{ __('Define ad-hoc service offerings with min/max pricing') }}">{{ __('Ad-hoc Services') }}</x-app.page-title>
        @can('adhoc_service.create')
        <a href="{{ route('adhoc_service.create') }}" class="bg-yellow-400 shadow rounded-md py-2 px-4 flex items-center gap-x-2">
...
@push('scripts')
    <script>
        ...
        var dt = new DataTable('#data-table', {
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        let editBtn = ''
                        let deleteBtn = ''
                        @can('adhoc_service.edit')
                        editBtn = `<a href="{{ config('app.url') }}/adhoc-service/edit/${row.id}" class="rounded-full p-2 bg-blue-200 inline-block" title="{!! __('Edit') !!}">
                        ...
                        @endcan
                        @can('adhoc_service.delete')
                        deleteBtn = `<button class="rounded-full p-2 bg-red-200 delete-btns" data-id="${row.id}" title="{!! __('Delete') !!}">
            ...
            ajax: {
                data: function(){
                    ...
                    var url = "{{ route('adhoc_service.get_data') }}"
        ...
        $('#data-table').on('click', '.delete-btns', function() {
            ...
            $('#delete-modal #yes-btn').attr('href', `{{ config('app.url') }}/adhoc-service/delete/${id}`)
