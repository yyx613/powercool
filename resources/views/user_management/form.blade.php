...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        <x-app.page-title
            url="{{ route('user_management.index') }}">{{ isset($user) ? __('Edit User') : __('Create User') }}</x-app.page-title>
    </div>
    @include('components.app.alert.parent')
    <form action="{{ isset($user) ? route('user_management.update', ['user' => $user]) : route('user_management.store') }}"
        ...
        <div class="bg-white p-4 rounded-md shadow" id="content-container">
            <div class="grid grid-cols-2 lg:grid-cols-3 gap-8 w-full mb-4">
                ...
                @if (isset($user))
                    <div class="flex flex-col">
                        <x-app.input.label id="sku" class="mb-1">{{ __('Staff ID') }} <span
                                class="text-sm text-red-500">*</span></x-app.input.label>
                        <x-app.input.input name="sku" id="sku" value="{{ isset($user) ? $user->sku : null }}"
                            disabled="true" />
                    </div>
                @endif
                <div class="flex flex-col">
                ...
                <div class="flex flex-col">
                    <x-app.input.label id="name" class="mb-1">{{ __('Username') }} <span
                            class="text-sm text-red-500">*</span></x-app.input.label>
                ...
                <div class="flex flex-col">
                    <x-app.input.label id="gender" class="mb-1">{{ __('Gender') }} <span
                            class="text-sm text-red-500">*</span></x-app.input.label>
                ...
                <div class="flex flex-col">
                    ...
                    <x-app.input.input name="state" id="state" :hasError="$errors->has('state')"
                        value="{{ old('state', isset($user) ? $user->state : null) }}" />
                ...
                <div class="flex flex-col">
                    ...
                    <x-app.input.input name="zip_code" id="zip_code" :hasError="$errors->has('zip_code')"
                        value="{{ old('zip_code', isset($user) ? $user->zip_code : null) }}" class="int-input" />
                    <x-input-error :messages="$errors->get('zip_code')" class="mt-1" />
                </div>
                <div class="flex flex-col">
                    <x-app.input.label id="password" class="mb-1">{{ __('Password') }} @if (!isset($user))
                ...
                <div class="flex flex-col">
                    <x-app.input.label id="password_confirmation" class="mb-1">{{ __('Password Confirmation') }}
                        @if (!isset($user))
                            <span class="text-sm text-red-500">*</span>
                        @endif
                    </x-app.input.label>
                    <x-app.input.input name="password_confirmation" type="password" id="password_confirmation"
                ...
                <div class="flex flex-col">
                    <x-app.input.label id="email" class="mb-1">{{ __('Email') }} @if (!isset($user))
                            <span class="text-sm text-red-500">*</span>
                        @endif
                    </x-app.input.label>
                ...
                <div class="flex flex-col">
                    ...
                    <x-app.input.input name="website" id="website" :hasError="$errors->has('website')"
                        value="{{ old('website', isset($user) ? $user->website : null) }}" />
                ...
                <div class="flex flex-col">
                    <x-app.input.label id="branch" class="mb-1">{{ __('Branch') }} <span
                            class="text-sm text-red-500">*</span></x-app.input.label>
                ...
                <div class="flex flex-col col-span-2">
                    ...
                    <x-app.input.input name="remark" id="remark" :hasError="$errors->has('remark')"
                        value="{{ old('remark', isset($user) ? $user->remark : null) }}" />
