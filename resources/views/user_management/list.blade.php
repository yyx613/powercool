...
@push('scripts')
    <script>
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return `<div class="flex items-center justify-end gap-x-2 px-2">
                            @can('user_role_management.edit')
                            <a href="{{ config('app.url') }}/user-management/edit/${row.id}" class="rounded-full p-2 bg-blue-200 inline-block" title="{!! __('Edit') !!}">
                            ...
                            @endcan
                            @can('user_role_management.delete')
                            <button class="rounded-full p-2 bg-red-200 inline-block delete-btns" data-id="${row.id}" data-name="${row.name}" title="{!! __('Delete') !!}">
        ...
        $('select[name="filter_role"]').on('change', function() {
            dt.draw()
        })
