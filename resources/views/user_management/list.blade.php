...
@section('content')
    ...
    <div>
        ...
        <!-- Table -->
        <table id="user-table" class="text-sm rounded-lg overflow-hidden" style="width: 100%;">
            <thead>
                <tr>
                    <th>No</th>
                    <th>Name</th>
...
@push('scripts')
    <script>
        ...
        // Datatable
        var userDataTable = new DataTable('#user-table', {
            bFilter: false,
            dom: 'rtip',
            pagingType: 'numbers',
            // ordering: false,
        ...
        function updateTableData(keyword='') {
            setTimeout(() => {
                let url = '{{ route("user_management.get_data") }}'
                            ...
                            for (let i = 0; i < res.length; i++) {
                                res[i].action = `
                                    <div class="flex items-center justify-end gap-x-2 px-2">
                                        <a href="{{ config('app.url') }}/user-management/edit/${res[i].id}" class="rounded-full p-2 bg-blue-200 inline-block">
                                        ...
                                        <button class="rounded-full p-2 bg-red-200 inline-block delete-btns" data-id="${res[i].id}" data-name="${res[i].name}">
                                ...
                                userDataTable.rows.add([res[i]]).draw();
                            }
                        } else {
                            userDataTable.draw();
                        }
        ...
        $('#user-table').on('click', '.delete-btns', function() {
            id = $(this).data('id')
            name = $(this).data('name')

            $('#delete-modal #yes-btn').attr('href', `{{ config('app.url') }}/user-management/delete/${id}`)
            $('#delete-modal #user-name').text(name)
