...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        <x-app.page-title
            url="{{ route('credit_term.index') }}">{{ isset($credit) ? __('Edit Credit Term') : __('Create Credit Term') }}</x-app.page-title>
    </div>
    @include('components.app.alert.parent')
    <form action="{{ isset($credit) ? route('credit_term.update', ['credit' => $credit]) : route('credit_term.store') }}"
            ...
            <div class="grid grid-cols-2 lg:grid-cols-3 gap-8 w-full mb-4">
                <div class="flex flex-col">
                    ...
                    <x-app.input.input name="name" id="name"
                        value="{{ old('name') ?? (isset($credit) ? $credit->name : null) }}" :hasError="$errors->has('name')" />
