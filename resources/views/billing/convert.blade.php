...
@section('content')
    <div class="mb-6">
        <x-app.page-title url="{{ route('invoice.index') }}">Convert Invoice to Billing</x-app.page-title>
    </div>
    <div class="flex gap-x-4">
        <!-- Left -->
        <div class="bg-white p-4 border rounded-md flex gap-x-14 flex-1">
            <!-- Steps -->
            ...
            <div class="flex-[3]">
                ...
                @if ($step == 1)
                    <div>
                        <div class="mb-2">
                            <h5 class="text-md font-semibold">Select invoice to proceed</h5>
                        </div>
                        @if (count($invoices) > 0)
                        ...
                        <div>
                            <form action="{{ route('billing.to_billing') }}">
                                ...
                                <div class="grid grid-cols-3 gap-8 w-full mb-8">
                                    ...
                                    <div class="flex flex-col">
                                        <x-app.input.label id="our_do_no" class="mb-1">Our D/O No</x-app.input.label>
        ...
        <div class="bg-white p-4 border rounded-md">
            <div class="max-h-44 overflow-y-auto">
                <table>
                    <thead>
                        <tr>
                            <th class="px-2 py-1 text-sm border-b">Product</th>
                            <th class="px-2 py-1 text-sm border-b">Qty/SKU</th>
                            <th class="px-2 py-1 text-sm border-b">U/Price</th>
                            <th class="px-2 py-1 text-sm border-b">T/Price</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($costs as $co)
                            <tr>
                                <td class="px-2 py-0.5 text-sm">{{ $co->product->model_name }}</td>
