            ...
            <tr>
                ...
                <td style="font-size: 14px; text-align: right;">
                    @php
                        ...
                        if ($custom_unit_price['custom-unit-price-' . $prod->id] != null) {
                            $unit_price = $custom_unit_price['custom-unit-price-' . $prod->id];
                        }
                    @endphp
                    {{ number_format($unit_price, 2) }}
                </td>
                <td style="font-size: 14px; text-align: right;">{{ number_format(0, 2) }}</td>
                <td style="font-size: 14px; text-align: right;">{{ number_format($prod->qty * $unit_price, 2) }}</td>
            </tr>
            @php
                $total += ($prod->qty * $unit_price);
            @endphp
        @endforeach
    </table>
    <!-- Item Summary -->
