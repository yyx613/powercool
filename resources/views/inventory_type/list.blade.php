...
@section('content')
    <div class="mb-3 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-2 md:mb-0" description="{{ __('Define product and raw material types by company group') }}">{{ __('Product Type') }}</x-app.page-title>
        <a href="{{ route('inventory_type.create') }}" class="bg-yellow-400 shadow rounded-md py-2 px-4 flex items-center gap-x-2">
...
@push('scripts')
    <script>
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                       return  `<div class="flex items-center justify-end gap-x-2 px-2">
                                <a href="{{ config('app.url') }}/inventory-type/edit/${row.id}" class="rounded-full p-2 bg-blue-200 inline-block" title="{!! __('Edit') !!}">
        ...
        $('#data-table').on('click', '.delete-btns', function() {
            ...
            $('#delete-modal #yes-btn').attr('href', `{{ config('app.url') }}/inventory-type/delete/${id}`)
