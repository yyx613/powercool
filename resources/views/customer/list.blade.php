...
@section('content')
    <div class="mb-3 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-2 md:mb-0" description="{{ __('Manage debtor profiles, categories, and sync with Autocount') }}">{{ __('Debtor') }}</x-app.page-title>
    ...
    <div>
        <!-- Filters -->
        <div class="flex flex-wrap gap-2 max-w-5xl w-full mb-4">
            <div class="flex-1">
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return `
                            ...
                            ${
                                row.can_duplicate ? `
                                    <a href="{{ config('app.url') }}/customer/create?id=${row.id}" class="rounded-full p-2 bg-yellow-200 inline-block" title="{!! __('Duplicate') !!}">
