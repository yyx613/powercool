    ...
    <form action="" method="POST" enctype="multipart/form-data" id="location-form">
        ...
        <div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 w-full mb-8 p-4 rounded-md relative hidden group transition durtion-300 hover:bg-slate-50"
                ...
                <div class="flex flex-col">
                    <x-app.input.label id="address4" class="mb-1">{{ __('Address 4') }} </x-app.input.label>
                    <x-app.input.input name="address4" id="address4" :hasError="$errors->has('address4')" />
        ...
        <div class="mt-8 flex justify-between">
            <!-- Add Items -->
            <button type="button"
                class="bg-yellow-400 rounded-md py-1.5 px-3 flex items-center gap-x-2 transition duration-300 hover:bg-yellow-300 hover:shadow {{ !isset($mode) ? '' : 'hidden' }}"
                id="add-item-btn">
    ...
    <script>
            ...
            if (CUSTOMER != null) {
                for (let i = 0; i < CUSTOMER.locations.length; i++) {
                    ...
                    if (loc.is_default == true) {
                        ...
                        else if (loc.type == 2) $(`.items[data-id="${i+1}"] .default-delivery-msg`).removeClass(
                            'hidden')
        ...
        $('#location-form').on('submit', function(e) {
            ...
            $('#location-form .items').each(function(i, obj) {
                ...
                address1.push($(this).find('input[name="address1"]').val())
                    ...
                    setTimeout(() => {
                        if (err.status == StatusCodes.UNPROCESSABLE_ENTITY) {
                            ...
                            for (const key in errors) {
                                ...
                                let idx = key.split('.')[1]
                                idx++
                                $(`#location-form .items[data-id="${idx}"] #${field}_err`).find(
                                    'p').text(errors[key])
                                $(`#location-form .items[data-id="${idx}"] #${field}_err`)
                                    .removeClass('hidden')
                            }
                        } else if (err.status == StatusCodes.BAD_REQUEST) {
                            $(`#location-form .items #is_default_err`).find('p').text(err
