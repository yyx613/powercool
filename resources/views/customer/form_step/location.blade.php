<div class="bg-white p-4 border rounded-md">
    ...
    <form action="" method="POST" enctype="multipart/form-data" id="location-form">
        ...
        <div class="mt-8 flex justify-between">
            <!-- Add Items -->
            <button type="button" class="bg-yellow-400 rounded-md py-1.5 px-3 flex items-center gap-x-2 transition duration-300 hover:bg-yellow-300 hover:shadow {{ !isset($mode) ? '' : 'hidden' }}" id="add-item-btn">
            ...
            <!-- Submit -->
            <x-app.button.submit id="submit-btn" class="{{ !isset($mode) ? '' : 'hidden' }}">{{ __('Save and Update') }}</x-app.button.submit>
        </div>
    </form>
</div>
