...
@section('content')
    <div class="mb-3 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-2 md:mb-0" :url="request('from') === 'approval' ? route('approval.index') : null" description="{{ __('Record goods received from suppliers and sync with Autocount') }}">{{ __('GRN') }}</x-app.page-title>
        @can('grn.create')
            ...
            columnDefs: [
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        var style = row.status != 1 ? 'style="opacity: 0.5; cursor: not-allowed;"' : '';
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                       return  `<div class="flex items-center justify-end gap-x-2 px-2">
                            ...
                            ${
                                row.can_cancel ?
                                `<button class="rounded-full p-2 bg-violet-200 inline-block cancel-btns" data-id="${row.sku}" title="{!! __('Cancel') !!}">
                            ...
                            ${
                                row.can_delete ?
                                `<button class="rounded-full p-2 bg-red-200 inline-block delete-btns" data-id="${row.sku}" title="{!! __('Delete') !!}">
                                    <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" id="Outline" viewBox="0 0 24 24" width="512" height="512"><path d="M21,4H17.9A5.009,5.009,0,0,0,13,0H11A5.009,5.009,0,0,0,6.1,4H3A1,1,0,0,0,3,6H4V19a5.006,5.006,0,0,0,5,5h6a5.006,5.006,0,0,0,5-5V6h1a1,1,0,0,0,0-2ZM11,2h2a3.006,3.006,0,0,1,2.829,2H8.171A3.006,3.006,0,0,1,11,2Zm7,17a3,3,0,0,1-3,3H9a3,3,0,0,1-3-3V6H18Zm-7-1a1,1,0,0,0,1-1V10a1,1,0,0,0-2,0v7A1,1,0,0,0,11,18Zm4,0a1,1,0,0,0,1-1V10a1,1,0,0,0-2,0v7A1,1,0,0,0,15,18ZM9,18a1,1,0,0,0,1-1V10A1,1,0,0,0,8,10v7A1,1,0,0,0,9,18Z"/></svg>
                                </button>` : ''
                            }
                       </div>`
                    }
