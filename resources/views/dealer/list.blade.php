...
@section('content')
    <div class="mb-6 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-4 md:mb-0">{{ __('Dealer') }}</x-app.page-title>
        <div class="flex gap-x-4">
            ...
            @can('customer.create')
            <a href="{{ route('dealer.create') }}" class="bg-yellow-400 shadow rounded-md py-2 px-4 flex items-center gap-x-2">
...
@push('scripts')
    <script>
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                       return  `<div class="flex items-center justify-end gap-x-2 px-2">
                            <a href="{{ config('app.url') }}/dealer/view/${row.id}" class="rounded-full p-2 bg-green-200 inline-block" title="{!! __('View') !!}">
                            ...
                            ${
                                row.can_edit ? `
                            <a href="{{ config('app.url') }}/dealer/edit/${row.id}" class="rounded-full p-2 bg-blue-200 inline-block" title="{!! __('Edit') !!}">
        ...
        $('#data-table').on('click', '.delete-btns', function() {
            ...
            $('#delete-modal #yes-btn').attr('href', `{{ config('app.url') }}/dealer/delete/${id}`)
            $('#delete-modal').addClass('show-modal')
        })
        $('#export-btn').on('click', function() {
            window.location.href = '{{ route("dealer.export") }}'
        })
    </script>
@endpush
