...
@section('content')
    <div class="mb-3 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-2 md:mb-0" description="{{ __('Define sales channels and e-invoice submission capability') }}">{{ __('Platform') }}</x-app.page-title>
        <a href="{{ route('platform.create') }}" class="bg-yellow-400 shadow rounded-md py-2 px-4 flex items-center gap-x-2">
...
@push('scripts')
    <script>
        ...
        var dt = new DataTable('#data-table', {
                ...
                { 
                    ...
                    render: function (data, type, row) {
                       return  `<div class="flex items-center justify-end gap-x-2 px-2">
                            <a href="{{ config('app.url') }}/platform/edit/${row.id}" class="rounded-full p-2 bg-blue-200 inline-block" title="{!! __('Edit') !!}">
            ...
            ajax: {
                data: function(){
                    ...
                    var url = "{{ route('platform.get_data') }}"
        ...
        $('#data-table').on('click', '.delete-btns', function() {
            ...
            $('#delete-modal #yes-btn').attr('href', `{{ config('app.url') }}/platform/delete/${id}`)
