...
<div {{ $attributes->class(['border-red-500' => $hasError, '!bg-gray-100' => $disabled, 'uppercase-input' => $uppercase])->merge(['class' => 'bg-white rounded-md border border-gray-300 overflow-hidden p-2 py-1.5']) }} data-removable="{{ $removable ? 'true' : 'false' }}">
    <div class="flex flex-wrap gap-1.5 " id="{{ $id }}_value_group">
        <div class="rounded-full bg-slate-100 pl-2 {{ !$removable ? 'pr-2' : '' }} items-center hidden {{ $id }}_values" data-id="0">
