<x-app.modal.base-modal id="supplier-sync-modal">
    <div class="aspect-[2/1] flex flex-col">
        <div class="border-b py-2 px-4 bg-gray-100">
            <h6 class="text-lg font-black" id="title">{{ __('Sync Confirmation') }}</h6>
        </div>
        <div class="flex-1 flex flex-col p-4">
            <div class="flex-1">
                <p class="text-slate-500 leading-snug">{{ __('Are you want to sync this supplier?') }}</p>
            </div>
...
@push('scripts')
<script>
    $('#supplier-sync-modal #no-btn').on('click', function() {
        $('#supplier-sync-modal').removeClass('show-modal')
    })
</script>
@endpush