<x-app.modal.base-modal id="transfer-so-modal">
    <div class="aspect-[2/1] flex flex-col">
        <div class="py-2 px-4 bg-emerald-100 flex items-center">
            ...
            <h6 class="text-lg font-black ml-3">{{ __('Transfer Sale Order') }}</h6>
        </div>
        <form method="POST" action="" class="flex-1 flex flex-col p-4 m-0">
            @csrf
            <div class="mb-8">
                <div class="mb-4">
                ...
                <div class="mb-4">
                    <span class="font-medium text-sm mb-1 block">{{ __('Sale Order') }}</span>
                    <div class="border px-2 py-1.5 rounded">
                        <p id="sale-order" class="text-sm"></p>
                    </div>
                </div>
                <div>
                    <span class="font-medium text-sm mb-1 block">{{ __('Branch to Transfer') }}</span>
                    <select name="branch" id="branch"
                        class="border border-slate-200 px-2 py-1.5 rounded w-full text-sm">
...
@push('scripts')
    <script>
        $('#transfer-so-modal #no-btn').on('click', function() {
            $('#transfer-so-modal').removeClass('show-modal')
            $('#transfer-so-modal #branch').val(null).trigger('change')
        })
        $('#transfer-so-modal #branch').on('change', function() {
            if (!$(this).val()) {
                $('#transfer-so-modal #yes-btn').addClass('hidden')
                return
            }
            $('#transfer-so-modal #yes-btn').removeClass('hidden')
        })
    </script>
@endpush
