<x-app.modal.base-modal id="transfer-back-sale-order-modal">
    <div class="aspect-[2/1] flex flex-col">
        <div class="py-2 px-4 bg-gray-100">
            <h6 class="text-lg font-black" id="title">{{ __('Transfer Back Confirmation') }}</h6>
        </div>
        <div class="flex-1 flex flex-col p-4 h-full">
            <div class="flex-1">
                <p class="text-slate-500 leading-snug">{{ __('Are you sure to transfer back the record?') }}</p>
            </div>
...
@push('scripts')
    <script>
        $('#transfer-back-sale-order-modal #no-btn').on('click', function() {
            $('#transfer-back-sale-order-modal').removeClass('show-modal')
        }) 
    </script>
@endpush
