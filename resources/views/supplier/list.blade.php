...
<div class="mb-6 flex justify-between items-center">
    <x-app.page-title>{{ __('Supplier') }}</x-app.page-title>
    @can('supplier.create')
...
<x-app.modal.supplier-sync-modal />
@endsection
...
@push('scripts')
    <script>
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return `<div class="flex items-center justify-end gap-x-2 px-2">
                                    ${row.can_edit ? `
                                    <a href="{{ config('app.url') }}/supplier/edit/${row.id}" class="rounded-full p-2 bg-blue-200 inline-block">
        ...
        $('#data-table').on('click', '.company1_sync', function () {
            ...
            $('#supplier-sync-modal #yes-btn').attr('href', `{{ config('app.url') }}/supplier/sync/${id}/HiTen`)
            $('#supplier-sync-modal').addClass('show-modal')
        })
        ...
        $('#data-table').on('click', '.company2_sync', function () {
            ...
            $('#supplier-sync-modal #yes-btn').attr('href', `{{ config('app.url') }}/supplier/sync/${id}/PowerCool`)
            $('#supplier-sync-modal').addClass('show-modal')
        })
    </script>
@endpush