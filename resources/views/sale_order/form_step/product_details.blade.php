    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-8 w-full mb-8 p-4 rounded-md relative group hidden transition durtion-300 hover:bg-slate-50"
        ...
        <div class="flex flex-col col-span-2 md:col-span-4">
            <div class="flex justify-between items-center mb-2">
                ...
                <button type="button" class="add-accessory-btn text-xs bg-blue-500 text-white px-3 py-1.5 rounded hover:bg-blue-600 hidden">+ {{ __('Add Accessory') }}</button>
            </div>
    ...
    <div class="mt-6 pt-6 border-t px-4">
        ...
        <div class="hidden" id="service-template">
            <div class="grid grid-cols-2 md:grid-cols-6 gap-4 lg:gap-8 w-full mb-4 p-4 rounded-md relative group transition duration-300 hover:bg-slate-50 service-item">
                ...
                <div class="flex flex-col">
                    <x-app.input.label class="mb-1">{{ __('Min Amount (RM)') }}</x-app.input.label>
                    <x-app.input.input name="adhoc_service_amount[]" class="service-amount" readonly />
                </div>
                <div class="flex flex-col">
                    <x-app.input.label class="mb-1">{{ __('Max Amount (RM)') }}</x-app.input.label>
                    <x-app.input.input name="adhoc_service_max_amount[]" class="service-max-amount" readonly />
                </div>
                <div class="flex flex-col">
...
@push('scripts')
    <script>
        ...
        $('#add-item-btn').on('click', function() {
            ...
            buildRemarkQuillEditor(ITEMS_COUNT)
        ...
        function loadExistingAccessories(itemId, accessories) {
            ...
            accessories.forEach(function(accessory) {
                ...
                if (accessory.is_foc == 1) {
                    ...
                    $newRow.find('.accessory-foc-btns').attr('data-is-foc', true)
                }
        ...
        $('body').on('change', '.adhoc-service-select', function() {
            ...
            if (service) {
                ...
                $(`.service-item[data-service-id="${serviceId}"] .service-max-amount input`).val(service.max_amount ? priceFormat(service.max_amount) : '-');
        ...
        function loadExistingAdhocServices() {
            if (SALE != null && SALE.adhoc_services && SALE.adhoc_services.length > 0) {
                ...
                for (let i = 0; i < SALE.adhoc_services.length; i++) {
                    ...
                    $(clone).find('.service-max-amount input').val(sas.adhoc_service.max_amount ? priceFormat(sas.adhoc_service.max_amount) : '-');
