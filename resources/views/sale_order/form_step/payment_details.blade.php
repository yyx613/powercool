<div class="bg-white p-4 border rounded-md" id="payment-details-container">
    <div class="flex items-center mb-6 border-l-8 border-yellow-400 px-3 py-1 bg-yellow-50 w-fit">
        ...
        <span class="text-lg ml-3 font-bold">{{ __('Payment Records') }}</span>
    </div>
        ...
        <div id="payment-amounts-container">
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 w-full p-4 hover:bg-slate-50 relative group hidden"
                ...
                <button type="button"
                    ...
                    title="{{ __('Delete Payment Amount') }}">
                ...
                if (SALE.payment_amounts.length <= 0) {
                    ...
                    for (let i = 0; i < SALE.payment_amounts.length; i++) {
                        ...
                        if (SALE.payment_amounts[i].approval_status == 2 || SALE.payment_amounts[i].approval_status == 3) {
                            $row.find('input, select').prop('disabled', true).css('backgroundColor', '#eee')
                            $row.find('input').parent().css('backgroundColor', '#eee')
                            $row.find('.select2').css('backgroundColor', '#eee')
