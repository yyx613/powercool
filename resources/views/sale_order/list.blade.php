...
@section('content')
    <div class="mb-3 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-2 md:mb-0" :url="request('from') === 'approval' ? route('approval.index') : null" description="{{ __('Track orders, convert from quotations, and generate delivery orders') }}">{{ __('Sale Order') }}</x-app.page-title>
...
@push('scripts')
    <script>
        ...
        var columnDefs = [{
                    ...
                    return `
                        ...
                        <div class="flex items-center gap-2 mt-1.5">
                            <div class="group relative">
                                ...
                                <div class="js-floating-tooltip fixed invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-opacity z-[9999] w-56">
                                ...
                                `<div class="group relative">
                                    ...
                                    <div class="js-floating-tooltip fixed invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-opacity z-[9999] w-56">
                                        <div class="rounded shadow border bg-white">
                                            ...
                                            <div class="px-3 py-2">${row.rejected_reason ?? '-'}</div>
                                        </div>
                                    </div>
                                </div>` : ''
                            }
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    return `<div class="flex flex-wrap w-32 items-center justify-end gap-2 px-2">
                        ${
                            row.can_to_sale_production_request && row.status != 3 ? `
                        ...
                        ${
                            row.can_transfer && !IS_SALES_ROLE && row.status != 3 ? `
                        ...
                        ${
                            row.can_edit_payment ? `<div class="group relative inline-block">
                                    <a href="{{ config('app.url') }}/sale-order/edit/payment/${row.id}?from=listing" class="rounded-full p-2 bg-amber-200 inline-block" title="{!! __('Payment') !!}">
                                    ...
                                    ${row.status == 3 && row.cancellation_charge != null && !row.has_payment_after_void ? `
                                        <span class="absolute top-0 right-0 h-3 w-3 bg-red-500 rounded-full border border-white"></span>
                                        <div class="group-hover:opacity-100 group-hover:z-10 absolute bottom-10 right-0 opacity-0 z-[-10] w-48">
                                            <div class="rounded shadow border bg-white p-2 text-xs">
                                                <p class="font-medium text-red-600">{!! __('Cancellation Charge') !!}: RM ${row.cancellation_charge}</p>
                                                <p class="text-gray-600 mt-1">{!! __('Please manually insert this charge into payment records.') !!}</p>
                                            </div>
                                        </div>
                                    ` : ''}
                                </div>` : ''
                        ...
                        ${
                            row.can_edit && row.status != 3 ? `
                                        <a href="{{ config('app.url') }}/sale-order/edit/${row.id}" class="rounded-full p-2 bg-blue-200 inline-block" title="{!! __('Edit') !!}">
                                            <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width="512" height="512"><path d="m18.813,10c.309,0,.601-.143.79-.387s.255-.562.179-.861c-.311-1.217-.945-2.329-1.833-3.217l-3.485-3.485c-1.322-1.322-3.08-2.05-4.95-2.05h-4.515C2.243,0,0,2.243,0,5v14c0,2.757,2.243,5,5,5h3c.552,0,1-.448,1-1s-.448-1-1-1h-3c-1.654,0-3-1.346-3-3V5c0-1.654,1.346-3,3-3h4.515c.163,0,.325.008.485.023v4.977c0,1.654,1.346,3,3,3h5.813Zm-6.813-3V2.659c.379.218.732.488,1.05.806l3.485,3.485c.314.314.583.668.803,1.05h-4.338c-.551,0-1-.449-1-1Zm11.122,4.879c-1.134-1.134-3.11-1.134-4.243,0l-6.707,6.707c-.755.755-1.172,1.76-1.172,2.829v1.586c0,.552.448,1,1,1h1.586c1.069,0,2.073-.417,2.828-1.172l6.707-6.707c.567-.567.879-1.32.879-2.122s-.312-1.555-.878-2.121Zm-1.415,2.828l-6.708,6.707c-.377.378-.879.586-1.414.586h-.586v-.586c0-.534.208-1.036.586-1.414l6.708-6.707c.377-.378,1.036-.378,1.414,0,.189.188.293.439.293.707s-.104.518-.293.707Z"/></svg>
                                        </a>` : ''
                        }
                        ${
                            row.can_view && row.status != 3 ? `
        ...
        $('#data-table').on('mouseenter', '.group.relative', function() {
            const $tooltip = $(this).find('> .js-floating-tooltip').first();
