...
@section('content')
    <div class="mb-3 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-2 md:mb-0" description="{{ __('Create support tickets, track status, and convert to tasks') }}">{{ __('Ticket') }}</x-app.page-title>
...
@push('scripts')
    <script>
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                       return  `<div class="flex items-center justify-end gap-x-2 px-2">
                            ${
                                row.status == 0 ? `` : `
                                    <button class="rounded-full p-2 bg-green-200 inline-block convert-btns" data-id="${row.id}" title="{!! __('Convert') !!}">
                            ...
                            ${
                                row.can_edit ? `
                            <a href="{{ config('app.url') }}/ticket/edit/${row.id}" class="rounded-full p-2 bg-blue-200 inline-block" title="{!! __('Edit') !!}">
        ...
        $('#data-table').on('click', '.delete-btns', function() {
            ...
            $('#delete-modal #yes-btn').attr('href', `{{ config('app.url') }}/ticket/delete/${id}`)
