            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-8 w-full mb-4">
                <div class="flex flex-col">
                    ...
                    <x-app.input.input name="name" id="name" :hasError="$errors->has('name')"
                        value="{{ old('name', isset($from_ticket) ? $from_ticket->subject : (isset($production) ? $production->name : null)) }}" />
                ...
                <div class="flex flex-col col-span-2 md:col-span-3">
                    ...
                    @foreach ($milestones as $stone)
                        @php
                            if (isset($production)) {
                                $productionMilestone = $prodMs
                                    ::where('production_id', $production->id)
                                    ->where('milestone_id', $stone->id)
                                    ->first();
                                $productionMilestonePreview = [];
                                if ($productionMilestone != null) {
                                    $productionMilestonePreview = $prodMsMaterialPreview
                                        ::where('production_milestone_id', $productionMilestone->id)
                                        ->get();
                                }
                            }
                        @endphp
                        <div class="flex justify-between mb-2 hidden milestone-selection" data-id="{{ $stone->id }}">
                            <div class="flex items-center gap-x-2">
                                <input type="checkbox" name="milestone[]" id="{{ $stone->id }}"
                                    value="{{ $stone->id }}" class="rounded-sm" @checked(in_array(
                                            $stone->id,
                                            old('milestone', isset($production) ? $production->milestones()->pluck('milestone_id')->toArray() : [])))>
                                <label for="{{ $stone->id }}" class="text-sm">{{ $stone->name }}</label>
                            </div>
                            <div class="flex items-center">
                                <button type="button" data-id="{{ $stone->id }}" data-is-custom="false"
                                    class="mr-3 view-material-use-selection-btns {{ isset($production) && count($productionMilestonePreview) > 0 ? '' : 'hidden' }}"
                                ...
                                <label
                                    ...
                                    <input type="checkbox" class="hidden peer" name="required_serial_no[]"
                                        data-id="{{ $stone->id }}" data-is-custom="false"
                                        @checked(isset($production) ? count($productionMilestonePreview) > 0 : null) />
    ...
    <script>
        ...
        $('select[name="product"]').on('change', function() {
            ...
            for (let i = 0; i < MILESTONES.length; i++) {
                if (MILESTONES[i].is_custom == false || (MILESTONES[i].is_custom == true && MILESTONES[i]
        ...
        $('body').on('change', 'input[name="required_serial_no[]"]', function() {
            ...
            if (id != undefined) {
                if (isCustom == false) {
                    if ($(this).is(':checked')) $(`.milestone-selection[data-id="${id}"]`).find('button')
                        .removeClass('hidden')
                    else $(`.milestone-selection[data-id="${id}"]`).find('button').addClass('hidden')
                } else {
                    if ($(this).is(':checked')) $(`.custom-milestone[data-id="${id}"]`).find('button').removeClass(
                        'hidden')
