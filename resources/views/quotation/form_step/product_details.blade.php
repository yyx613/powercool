<div class="bg-white p-4 border rounded-md" id="product-details-container">
    ...
    <!-- Add Items -->
    <div class="flex justify-end px-4 {{ isset($sale) && $sale->status == 2 ? 'hidden' : '' }}">
        <button type="button"
    ...
    <div class="mt-6 pt-6 border-t px-4">
        <div class="flex items-center justify-between mb-4">
            ...
            <button type="button"
                class="bg-green-700 text-white rounded-md py-1.5 px-3 flex items-center gap-x-2 transition duration-300 hover:bg-green-600 hover:shadow {{ isset($sale) && $sale->status == 2 ? 'hidden' : '' }}"
                id="add-service-btn">
        ...
        <div id="services-empty-state" class="text-center py-8 bg-slate-50 border border-dashed border-slate-300 rounded mb-4">
            ...
            <p class="text-xs text-slate-400">{{ __('Click "Add Service" button to add services') }}</p>
        </div>
    </div>
        ...
        $(document).ready(function() {
            if (SALE != null) {
                for (let i = 0; i < SALE.products.length; i++) {
                    ...
                    let opt = new Option(`${sp.product.sku} - ${sp.product.model_desc}`, sp.product_id, true, true);
