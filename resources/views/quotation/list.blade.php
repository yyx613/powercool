            ...
            },
            {
                ...
                render: function(data, type, row) {
                    return `<div class="flex flex-wrap w-32 items-center justify-end gap-2 px-2">
                        ...
                        ${
                            row.can_view_pdf ? `
                                                                                                <a href="{{ config('app.url') }}/quotation/pdf/${row.id}${row.is_preview_pdf ? '#toolbar=0&navpanes=0' : ''}" class="rounded-full p-2 bg-yellow-200 inline-block" target="_blank" title="{!! __('View PDF') !!}">
