...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        <x-app.page-title url="{{ route('currency.index') }}">{{ isset($curr) ? __('Edit Currency') : __('Create Currency') }}</x-app.page-title>
    </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-3 gap-8 w-full mb-4">
                ...
                <div class="flex flex-col">
                    ...
                    <x-app.input.select name="status" id="status" :hasError="$errors->has('status')">
                        ...
                        <option value="1" @selected(old('status', isset($curr) ? $curr->is_active : null) == 1)>{{ __('Active') }}</option>
