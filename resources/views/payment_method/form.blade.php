...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        <x-app.page-title url="{{ route('payment_method.index') }}">{{ isset($method) ? __('Edit Payment Method') : __('Create Payment Method') }}</x-app.page-title>
    </div>
    @include('components.app.alert.parent')
    <form action="{{ isset($method) ? route('payment_method.update', ['method' => $method]) : route('payment_method.store') }}" method="POST" enctype="multipart/form-data" id="form">
        @csrf
