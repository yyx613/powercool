@extends('layouts.app')
@section('title', 'Consolidated E-Invoice')
...
@section('content')
    <div class="mb-3 flex justify-between items-start md:items-center flex-col md:flex-row">
        <x-app.page-title class="mb-2 md:mb-0" description="{{ __('View consolidated invoices and submit credit/debit notes') }}">{{ __('Consolidated E Invoice') }}</x-app.page-title>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('to_note', ['from' => 'cons']) }}" class="bg-green-200 shadow rounded-md py-2 px-4 flex items-center gap-x-2" id="submit-credit-btn1">
        ...
        var dt = new DataTable('#data-table', {
                ...
                { 
                    ...
                    render: function (data, type, row) {
                       return  `<div class="flex items-center justify-end gap-x-2 px-2">
                            <a href="{{ config('app.url') }}/e-invoice/download?uuid=${row.uuid}&type=consolidated" class="rounded-full p-2 bg-green-200 inline-block" target="_blank" title="{!! __('Download') !!}">
            ...
            ajax: {
                data: function(){
                    ...
                    var url = "{{ route('invoice.get_data_consolidated-e-invoice') }}"
