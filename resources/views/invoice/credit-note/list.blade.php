...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        <x-app.page-title>{{ __('Credit Note') }}</x-app.page-title>
    </div>
        ...
        var dt = new DataTable('#data-table', {
                ...
                { 
                    ...
                    render: function (data, type, row) {
                        ...
                        return  `<div class="flex items-center justify-end gap-x-2 px-2">
                            <a href="{{ config('app.url') }}/e-invoice/download?uuid=${row.uuid}&type=credit" class="rounded-full p-2 bg-green-200 inline-block" target="_blank" title="{!! __('Download') !!}">
                            ...
                            <button class="send-to-customer-btn rounded-full p-2 bg-green-200 inline-block"
                                data-id="${row.id}" data-type="credit" title="{!! __('Send Email') !!}">
            ...
            ajax: {
                data: function(){
                    ...
                    var url = "{{ route('invoice.get_data_credit-note') }}"
