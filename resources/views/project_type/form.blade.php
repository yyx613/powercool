...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        <x-app.page-title url="{{ route('project_type.index') }}">{{ isset($pt) ? __('Edit Project Type') : __('Create Project Type') }}</x-app.page-title>
    </div>
    @include('components.app.alert.parent')
    <form action="{{ isset($pt) ? route('project_type.update', ['type' => $pt]) : route('project_type.store') }}" method="POST" enctype="multipart/form-data" id="form">
        @csrf
            ...
            <div class="grid grid-cols-3 gap-8 w-full mb-4">
                ...
                <div class="flex flex-col">
                    ...
                    <x-app.input.select name="status" id="status" :hasError="$errors->has('status')">
                        ...
                        <option value="1" @selected(old('status', isset($pt) ? $pt->is_active : null) == 1)>{{ __('Active') }}</option>
                        <option value="0" @selected(old('status', isset($pt) ? $pt->is_active : null) === 0)>{{ __('Inactive') }}</option>
                    </x-app.input.select>
